...
class PoCFedAvg(FedAvg):
    """Power-of-Choice client selection algorithm based on FedAvg framework (not pFedMe)"""
    ...
    def train(self, save_model=False, current_time=0, total_times=1):
        for glob_iter in range(self.num_glob_iters):
            ...
            logger.info(f"-------------[{current_time+1}/{total_times}] Round: {glob_iter+1}/{self.num_glob_iters} (PoCFedAvg)-------------")
